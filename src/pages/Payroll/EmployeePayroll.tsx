...
const EmployeePayroll: React.FC = () => {
    ...
    const [employees, setEmployees] = useState<Employee[]>([]); // Để lấy thông tin tên nhân viên
    ...
    const handleGetMyPayrollByMonth = async () => {
        ...
        try {
            ...
            setPage(0); // Reset về trang đầu tiên khi xem theo tháng
    ...
    useEffect(() => {
        // Fetch all employees once to resolve employee names
    ...
    // Helper function to display employee name from ID
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
                                <thead className="bg-gray-50 dark:bg-gray-900">
                                    <tr className="dark:bg-gray-900">
                                        <th className="w-[30%] px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                            Kỳ lương
                                        </th>
                                        <th className="w-[25%] px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                            Lương cơ bản
                                        </th>
                                        <th className="w-[25%] px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">
                                            Hoa hồng
                                ...
                                <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                                    {isLoading ? (
                                        ...
                                        payrolls.slice(page * pageSize, (page + 1) * pageSize).map((payroll) => (
                                            <tr key={payroll.payrollId} className="border-t border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">
                                                ...
                                                <td className="py-3 px-4 whitespace-nowrap dark:text-gray-100">
                                                ...
                                                <td className="py-3 px-4 whitespace-nowrap dark:text-gray-100 text-center">
                                                    {payroll.totalAppointments}
                                                </td>
                                                <td className="py-3 px-4">
                                                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                                                        payroll.status === 'PAID' ? 'bg-green-100 text-green-800 dark:bg-green-700 dark:text-green-100' :
                                                            payroll.status === 'APPROVED' ? 'bg-blue-100 text-blue-800 dark:bg-blue-700 dark:text-blue-100' :
                                                                payroll.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-700 dark:text-yellow-100' :
                                                                    payroll.status === 'DRAFT' ? 'bg-gray-200 text-gray-700 dark:bg-gray-600 dark:text-gray-200' :
